...
class RejectProduct extends StatefulWidget {
  const RejectProduct({super.key});

  @override
  State<RejectProduct> createState() => _RejectProductState();
}

class _RejectProductState extends State<RejectProduct> {
  ...
  Future<void> fetchRejectedOrders() async {
    try {
      ...
      final response = await http.get(
        Uri.parse('https://moodpandaa.com/public/api/vendor/reject-orders'),
        headers: {
      ...
      if (response.statusCode == 200) {
        ...
        if (responseData['success'] == true && responseData['data'] != null) {
          ...
          setState(() {
            errorMessage = "No rejected orders found!";
            isLoading = false;
          });
        }
      } else {
        setState(() {
          errorMessage = "Failed to load rejected orders!";
          isLoading = false;
        });
      }
    } catch (error) {
